...
def file_exists_nonempty(path: str):
...
def load_tx_agent(save_rl_model_dir):
    tx_agent = Actor(5)
...
def take_agent_action(val_ds, actor, snr_values=None):
    if snr_values is None:
        snr_values = list(range(0, 60, 1))
    ...
    resize_imgs  = tf.image.resize(imgs, (8,8))
...
def compute_snr_ranges(arr, num_levels=5):
    snr_ranges = {}
    index = 0
    for val, group in groupby(arr):
        length = sum(1 for _ in group)
        snr_ranges[val] = (index, index + length)
        index += length
    for v in range(num_levels):
        snr_ranges.setdefault(v, (None, None))
    return snr_ranges

def get_snr_ranges(val_dataset, tx_agent, args):
    prof = 'NTN-TDL-D'
    ...
    best_actions = take_agent_action(val_ds_prof, tx_agent, snr_values=None)
    snr_ranges = compute_snr_ranges(best_actions, num_levels=5)
    return snr_ranges
...
def serialize_example(symbols, noisy_symbols, snr, mod_idx):
    symbols_real = tf.io.serialize_tensor(tf.math.real(symbols))
    symbols_imag = tf.io.serialize_tensor(tf.math.imag(symbols))
    noisy_real   = tf.io.serialize_tensor(tf.math.real(noisy_symbols))
    noisy_imag   = tf.io.serialize_tensor(tf.math.imag(noisy_symbols))
    
    feature = {
        'symbols_real': _bytes_feature(symbols_real),
        'symbols_imag': _bytes_feature(symbols_imag),
        'noisy_real':   _bytes_feature(noisy_real),
        'noisy_imag':   _bytes_feature(noisy_imag),
...
def save_train_symbol(dataset, vqvae_model, snr_ranges, args):
    len_syms = 960 # (64 (M) * 16 (N)) - (16 (zero_padded) * 4 (column_num)) = 1024 - 64 = 960
    batch_size = args.batch_size
    profile = 'NTN-TDL-D'

    filename = os.path.join(args.save_dir, "doppler_symbol_train_dataset.tfrecord")
    if file_exists_nonempty(filename):
        print(f"[SKIP] Train symbol tfrecord already exists: {filename}")
        return
    ...
    with tf.io.TFRecordWriter(filename) as writer:
        ...
        for step in tqdm(range(num_batches)):
            images = next(ds_iter)
            # mod = np.random.randint(0, 5)  
            mod    = mods[step]     
            snr_min, snr_max = snr_ranges[mod]

            snr_vals = tf.random.uniform(
                shape=[batch_size], minval=snr_min, maxval=snr_max,
                dtype=tf.int32
            )
            modulation_index = tf.constant(mod, dtype=tf.int32)
            M_mod = modulation_schemes[mod]['modulation_order']
            modulate_fn = modulation_schemes[mod]['modulate_fn']
            ...
            snr_per_sample = tf.reshape(snr_vals, [batch_size])

            noisy_frames = tf.map_fn(
            ...
            example = serialize_example(symbols_re, noisy_symbols, snr_vals.numpy().flatten().tolist(), int(modulation_index.numpy()))
            writer.write(example)
    print(f"All modulation data saved to {filename}")

def save_val_symbol(dataset, vqvae_model, snr_ranges, args):
    len_syms = 960 # (64 (M) * 16 (N)) - (16 (zero_padded) * 4 (column_num)) = 1024 - 64 = 960
    batch_size = args.batch_size
    profile = 'NTN-TDL-D'
    ...
    filename = os.path.join(args.save_dir, "doppler_symbol_val_dataset.tfrecord")
    if file_exists_nonempty(filename):
        print(f"[SKIP] Val symbol tfrecord already exists: {filename}")
        return
    ...
    with tf.io.TFRecordWriter(filename) as writer:
        ...
        for step in tqdm(range(num_batches)):
            ...
            noisy_symbols = tf.reshape(noisy_frames, [batch_size, -1])[:, :sym_len]
            example = serialize_example(symbols, noisy_symbols, snr_vals.numpy().flatten().tolist(), int(modulation_index.numpy()))
...
def main(args):
    ...
    train_path = os.path.join(save_dir, "doppler_symbol_train_dataset.tfrecord")
    val_path   = os.path.join(save_dir, "doppler_symbol_val_dataset.tfrecord")

    if file_exists_nonempty(train_path) and file_exists_nonempty(val_path):
        print(f"[SKIP] Both train/val symbol tfrecords already exist in {save_dir}")
        sys.exit(0)
    ...
    tx_agent = load_tx_agent(args.save_rl_model_dir)
    snr_ranges = get_snr_ranges(val_dataset, tx_agent, args)

    if not file_exists_nonempty(train_path):
        save_train_symbol(train_dataset, vqvae_model, snr_ranges, args)
    else:
        print(f"[SKIP] Train tfrecord already exists: {train_path}")

    if not file_exists_nonempty(val_path):
        save_val_symbol(val_dataset, vqvae_model, snr_ranges, args)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_rl_model_dir", type=str, default=config.get('rl_model_dir', f'./rl_model/{args.dataset_name}'))
